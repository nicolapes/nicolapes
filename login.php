...
<?php
	if(isset($_POST['login'])){
		...
		$result = $mysqli->query("SELECT * FROM user WHERE username LIKE '$username' AND password='$password'");
		
		if(($result->num_rows) > 0){
			$result = $mysqli->query("SELECT username FROM user WHERE username LIKE '$username'");
			$row = $result->fetch_row();
			$username = $row[0];
			$result = $mysqli->query("SELECT id FROM user WHERE username LIKE '$username'");
			$row = $result->fetch_row();
			$userid = $row[0];
			...
			$result = $mysqli->query("SELECT * FROM user WHERE username LIKE '$username'");
			if(($result->num_rows) > 0) {
